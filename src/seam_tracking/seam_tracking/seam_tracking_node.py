...
class SeamTracking(Node):
    ...
    def __init__(self):
        ...
        self.srv_set_codes = self.create_service(
            ...
            '~/set_codes',
            self._cb_set_codes)
        self.srv_count_codes = self.create_service(
            CountCodes,
            '~/count_codes',
            self._cb_count_codes)
    ...
    def _cb_get_codes(self, request, response):
        try:
            response.codes, response.index = self.codes.get_codes()
        except Exception as e:
            response.success = False
            response.message = str(e)
        else:
    ...
    def _cb_sub(self, msg: PointCloud2):
        if msg.data:
            ...
            try:
                ...
                d = np.array(list(zip(a, b)), dtype=[('x', np.float32), ('y', np.float32)])
                if d.size > 0:
